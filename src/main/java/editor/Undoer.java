...
public class Undoer {
  ...
  void doUndo(LevelWindow lw) {
    if (undo.size() > 0) {
      ...
      if (undo.size() == 0) lw.m_parent.m_main.editUndo.setEnabled(false);
    } else {
      lw.m_parent.m_main.editUndo.setEnabled(false);
    }
  }
  ...
  void doRedo(LevelWindow lw) {
    if (redo.size() > 0) {
      ...
      if (redo.size() == 0) lw.m_parent.m_main.editRedo.setEnabled(false);
  ...
  void snapShot(LevelWindow lw) {
    ...
    if (s != null && t == null) {
      ...
      showUndoHideRedo(lw);
    } else if (s != null && t != null) { // dual state save
      Vector v = new Vector();
  ...
  static boolean isSameVecArray(Vector one, Vector two) {
    ...
    if (one == null) one = new Vector();

    if (two == null) two = new Vector();
  ...
  class Step {
    ...
    void undo(LevelWindow lw) {
      if (type == TYPE_SETTILES) {
        ...
        if (lw.width > 0) lw.height = ((Vector) data.get(0)).size();
        else lw.height = 0;
