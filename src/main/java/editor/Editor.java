...
public class Editor extends JScrollPane {
  ...
  /** Creates a new editor instance, this constructor should be called for a new lvl file. */
  ...
  public boolean load() {
    ...
    try {
      ...
      if (m_lvlFile.regions != null) m_window.m_asssRegions.setRegions(m_lvlFile.regions);
      ...
      // setup exists
      {
        ...
        if (m_window.m_autotool.isEmpty()) { // Process Map
          autoDetectWall(m_map, m_window.m_autotool.getAutoTileset());
        }
      }
      pf.hide();
  ...
  public void save() {
    ...
    if (attNode != null) setup.children.add(attNode);
   ...
   * @param wall where the wall will be stored
   */
   ...
   *
   * @param t_up the tile above ours
   * @param t_down the tile below ours
   * @param t_left the tile left of ours
   * @param t_right the tile right of ours
    ...
    for (int x = 1; x < 1023; ++x)
      for (int y = 1; y < 1023; ++y) {
        ...
        if (value != 0) {
          ...
          if (right == t_right && up == t_up && left == t_left && down == t_down) {
