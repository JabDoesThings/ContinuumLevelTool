...
public class TilesetWindow extends JInternalFrame implements MouseListener {
  ...
  /**
   * Constructor for TilesetWindow which displays the current tileset. It reads the tileset from
   * Editor.java which keeps it up to date even when the tileset may change through import or edit.
   *
   ...
   *
   * @param button Button pressed from MouseEvent
  ...
  public void mousePressed(MouseEvent e) {
    ...
    if (helpRect.contains(x, y)) {
      ...
      JOptionPane.showMessageDialog(
          null, "", "Tools Help", JOptionPane.INFORMATION_MESSAGE, helpIcon);
      ...
      } else if (x_coordL > 18) // attempting to select erase / special tile
      {
        ...
        if (y_coordL != 0) x_coordL = 19;
        else {
          if (x > 328) x_coordL = 20;
          else x_coordL = 19;
        }
      }

      if (x_coordL < 0) x_coordL = 0;

      if (y_coordL < 0) y_coordL = 0;
      ...
      } else if (x_coordR > 18) // attempting to select erase / special tile
      {
        ...
        if (y_coordR != 0) x_coordR = 19;
        else {
          if (x > 328) x_coordR = 20;
          else x_coordR = 19;
        }
      }

      if (x_coordR < 0) x_coordR = 0;

      if (y_coordR < 0) y_coordR = 0;
    }
  ...
  public int getTile(int button) {
    if (button == MouseEvent.BUTTON1 || button == 0) {
      ...
      // erase tool or special tile
      {
        ...
        {
          if (x_coordL == 19) return TilesetWindow.TILE_EZ;
          else return 0; // erase
        } else if (y_coordL == 1) return 216; // small asteriod
        else if (y_coordL == 2) return 217; // medium asteriod
        else if (y_coordL == 3) return 219; // station
        else if (y_coordL == 4) return 220; // wormhole
      }
    } else {
      if (x_coordR < 19) {
      ...
      // erase tool or special tile
      {
        ...
        {
          if (x_coordR == 19) return TilesetWindow.TILE_EZ;
          else return 0; // erase
        } else if (y_coordR == 1) return 216; // small asteriod
        else if (y_coordR == 2) return 217; // medium asteriod
        else if (y_coordR == 3) return 219; // station
        else if (y_coordR == 4) return 220; // wormhole
      }
    }
