...
public class Main extends JFrame implements ActionListener, ChangeListener, WindowListener {
  ...
  private JMenuItem fileNew, fileOpen, fileClose, fileSave, fileSaveAs, fileExit, fileSaveAll;
  public JMenuItem editDelete, editCut, editCopy, editPaste, editUndo, editRedo, editTileset;
  private JMenuItem viewZoomIn, viewZoomOut;
  private JMenuItem style1, style2, style3, style4, style5, style6;
  public JMenuItem windowRadar, windowTileset, windowAutoTile, windowLvz, windowAsssRegions;
  private JMenuItem helpTools, helpLvz, helpAbout, helpManual;

  private JMenuItem eLvlAttrs;

  private JMenuItem flipHoriz, flipVert, rotate, retile, applyEzTile, randomFill, generateScript;
  ...
  public Main() {
    ...
    if (strClassPath == null) Main.rootDirectory = new File(".").getAbsolutePath();
    else {
      int intIndex = strClassPath.indexOf(File.separator);

      if (intIndex != -1) strClassPath = strClassPath.substring(0, intIndex);
      ...
      if (strClassPath.matches("(?i:.*\\.jar)")) {
        ...
        if (objCurDir == null) objCurDir = new File(".");
  ...
  private void init() {
    ...
    randomFill = new JMenuItem("Randomly Fill Selection With Selected Tile (by %)");
    randomFill.addActionListener(this);
    selection.add(randomFill);

    generateScript = new JMenuItem("Generate Script");
    ...
    viewZoomIn.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_UP, ActionEvent.CTRL_MASK));
    ...
    viewZoomOut.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, ActionEvent.CTRL_MASK));
    ...
    helpTools = new JMenuItem("What are all of these tools, and how do I use them?");
  ...
  private void load() {
    ...
    try {
      ...
      for (int i = 1; i < 6; i++) {
        ...
        if (i == 1 || i == 3) extras[i - 1] = b.getImage(16);
        else if (i == 2) extras[i - 1] = b.getImage(32);
        else if (i == 4) extras[i - 1] = b.getImage(96);
        else if (i == 5) extras[i - 1] = b.getImage(80);
      }
    ...
    XMLNode doc =
        XMLNode.readDocumentNoError(
            Main.rootDirectory + File.separator + "include" + File.separator + "preferences.xml");
    ...
    if (doc != null) {
      ...
      if (doc == null) {
        JOptionPane.showMessageDialog(
            null, "Preferences.xml has been tampered with, no Preferences Node.");
      } else {
        ...
        if (x != null) ps.pos_AsssRegions.x = x.getIntValue();

        x = doc.getChild("loc_asssRegions_y");
        if (x != null) ps.pos_AsssRegions.y = x.getIntValue();

        x = doc.getChild("loc_autoTile_x");
        if (x != null) ps.pos_Autotool.x = x.getIntValue();

        x = doc.getChild("loc_autoTile_y");
        if (x != null) ps.pos_Autotool.y = x.getIntValue();

        x = doc.getChild("loc_lvzImages_x");
        if (x != null) ps.pos_LvzImages.x = x.getIntValue();

        x = doc.getChild("loc_lvzImages_y");
        if (x != null) ps.pos_LvzImages.y = x.getIntValue();

        x = doc.getChild("loc_radar_x");
        if (x != null) ps.pos_Radar.x = x.getIntValue();

        x = doc.getChild("loc_radar_y");
        if (x != null) ps.pos_Radar.y = x.getIntValue();

        x = doc.getChild("loc_tileset_x");
        if (x != null) ps.pos_Tileset.x = x.getIntValue();

        x = doc.getChild("loc_tileset_y");
        if (x != null) ps.pos_Tileset.y = x.getIntValue();

        x = doc.getChild("confirm_elvl");
        if (x != null) confirmELVL = x.getIntValue() != 0;
        ...
        Rectangle bounds =
            new Rectangle(
                inset, inset, screenSize.width - inset * 2, screenSize.height - inset * 2);

        x = doc.getChild("window_x");
        if (x != null) {
          int loc = x.getIntValue();
          if (loc > 0) bounds.x = loc;
          else bounds.x = 0;
        }
        ...
        if (x != null) {
          int loc = x.getIntValue();
          if (bounds.x + loc > screenSize.width) bounds.width = screenSize.width - bounds.x;
          else bounds.width = loc;
        }

        x = doc.getChild("window_h");
        if (x != null) {
          int loc = x.getIntValue();
          if (bounds.y + loc > screenSize.height) bounds.height = screenSize.height - bounds.y;
          else bounds.height = loc;
        }

        setBounds(bounds.x, bounds.y, bounds.width, bounds.height);
      }
    } else { // doc == null, first time running it
      JOptionPane.showMessageDialog(
          null,
      ...
      Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
      Rectangle bounds =
          new Rectangle(inset, inset, screenSize.width - inset * 2, screenSize.height - inset * 2);
  ...
  private void doScriptWork() {
    ...
    if (index != -1) {
      ...
      ld.setText(s);
      ld.setVisible(true);
    } else JOptionPane.showMessageDialog(null, "You don't have a map open.");
  }
  ...
  public void randomFill() {
    ...
    if (index != -1) {
      String s =
          JOptionPane.showInputDialog(null, "How much percent do you want to be filled?", "5.0");
      ...
      if (s != null) {
        try {
          ...
          if (selectedTile == TilesetWindow.TILE_EZ) {
            JOptionPane.showMessageDialog(
                null, "You can't fill by percent with the EZ tile. Sorry.");
          } else {
            if (selection != null) {
              for (int x = 0; x < selection.size(); ++x) {
                ...
                for (int y = 0; y < col.size(); ++y) {
                  if (Math.random() * 100 < d) col.setElementAt(new Integer(selectedTile), y);
                }
              }
            }
          ...
          JOptionPane.showMessageDialog(null, "Error parsing string into number: " + s);
        }
      }
    }
  }
  ...
  public void undo() {
    ...
    } else editUndo.setEnabled(false);
  }
  ...
  public void redo() {
    ...
    } else editRedo.setEnabled(false);
  }
  ...
  public void rotateByAngle() {
    ...
    if (index != -1) {
      ...
      if (selection != null) {
        String s =
            JOptionPane.showInputDialog(
                null, "How many degrees clockwise should we rotate?", "90.0");
        ...
        if (s != null) {
          try {
            ...
            if (e.m_window.width > 0) {
              e.m_window.height = ((Vector) selection.get(0)).size();
            }
    ...
    if (index != -1) {
      ...
      if (selection != null) {
        ...
        if (n == JOptionPane.YES_OPTION) {
          ...
          if (to == TilesetWindow.TILE_EZ || from == TilesetWindow.TILE_EZ) {
            JOptionPane.showMessageDialog(null, "You can't swap tiles using the EZ tile. Sorry.");
          } else {
            ...
            for (int x = 0; x < selection.size(); ++x) {
              ...
              for (int y = 0; y < col.size(); ++y) {
                ...
                if (tile == from) col.setElementAt(new Integer(to), y);
              }
            }
   ...
   *
   * @param x the x position in the vector array
   * @param y the y position
   * @param v the vector array
   * @return the integer stored in the Integer in this position in the vector array
   */
  int getIntFromVecArray(int x, int y, Vector v) {
    int rv;

    if (x == -1 || y == -1 || v.size() == 0) rv = 0;
            ...
            if (apply) {
              ...
              for (int nx = -1; nx < 2; ++nx) {
                for (int ny = -1; ny < 2; ++ny) {
                  nebs[nx + 1][ny + 1] = getIntFromVecArray(x + nx, y + ny, selection);
                }
              }

              int should = Tools.getWhatAutoTileShouldBe(nebs, autoSet);
  ...
  public void actionPerformed(ActionEvent ae) {
    if (ae.getSource() == fileExit) close();
    else if (ae.getSource() == fileSaveAs) cmdSaveAs();
    else if (ae.getSource() == fileSaveAll || ae.getSource() == managerSaveAll) cmdSaveAll();
    else if (ae.getSource() == fileSave || ae.getSource() == managerSave) cmdSave();
    else if (ae.getSource() == fileNew || ae.getSource() == managerNew) cmdNewFile();
    else if (ae.getSource() == fileOpen || ae.getSource() == managerOpen) cmdOpenFile();
    else if (ae.getSource() == fileClose) cmdCloseFile();
    else if (ae.getSource() == editTileset) cmdEditTileset();
    else if (ae.getSource() == viewZoomIn) cmdZoomIn();
    else if (ae.getSource() == viewZoomOut) cmdZoomOut();
    else if (ae.getSource() == buttonPen) setButtons(buttonPen, Tools.PEN);
    else if (ae.getSource() == buttonSelect) setButtons(buttonSelect, Tools.SELECT);
    else if (ae.getSource() == buttonEyedrop) setButtons(buttonEyedrop, Tools.EYEDROP);
    else if (ae.getSource() == buttonAutoPen) setButtons(buttonAutoPen, Tools.AUTO_PEN);
    else if (ae.getSource() == buttonLvz) setButtons(buttonLvz, Tools.LVZ);
    else if (ae.getSource() == buttonLvzSelection)
      setButtons(buttonLvzSelection, Tools.LVZ_SELECTION);
    else if (ae.getSource() == buttonZoom) setButtons(buttonZoom, Tools.ZOOM);
    else if (ae.getSource() == buttonRgn) setButtons(buttonRgn, Tools.RGN);
    else if (ae.getSource() == windowRadar) showWindow(RADAR_WINDOW);
    else if (ae.getSource() == windowTileset) showWindow(TILESET_WINDOW);
    else if (ae.getSource() == windowAutoTile) showWindow(AUTOTILE_WINDOW);
    else if (ae.getSource() == windowLvz) showWindow(LVZ_WINDOW);
    else if (ae.getSource() == windowAsssRegions) showWindow(REGION_WINDOW);
    else if (ae.getSource() == style1) setStyle(1);
    else if (ae.getSource() == style2) setStyle(2);
    else if (ae.getSource() == style3) setStyle(3);
    else if (ae.getSource() == style4) setStyle(4);
    else if (ae.getSource() == style5) setStyle(5);
    else if (ae.getSource() == style6) setStyle(6);
    else if (ae.getSource() == editCopy) copy();
    else if (ae.getSource() == editCut) cut();
    else if (ae.getSource() == editDelete) deleteSelection();
    else if (ae.getSource() == editPaste) paste();
    else if (ae.getSource() == helpTools) help_tools();
    else if (ae.getSource() == helpLvz) help_lvz();
    else if (ae.getSource() == helpManual) help_manual();
    else if (ae.getSource() == helpAbout) help_about();
    else if (ae.getSource() == buttonLine) setButtons(buttonLine, Tools.LINE);
    else if (ae.getSource() == buttonSquare) setButtons(buttonSquare, Tools.SQUARE);
    else if (ae.getSource() == buttonSquareFill) setButtons(buttonSquareFill, Tools.SQUAREFILL);
    else if (ae.getSource() == buttonEllipse) setButtons(buttonEllipse, Tools.ELLIPSE);
    else if (ae.getSource() == buttonEllipseFill) setButtons(buttonEllipseFill, Tools.ELLIPSEFILL);
    else if (ae.getSource() == buttonFill) setButtons(buttonFill, Tools.FILL);
    else if (ae.getSource() == lastMI[0]) loadFileFromPath(lastMaps[0]);
    else if (ae.getSource() == lastMI[1]) loadFileFromPath(lastMaps[1]);
    else if (ae.getSource() == lastMI[2]) loadFileFromPath(lastMaps[2]);
    else if (ae.getSource() == showGrid) {
      gridOn = showGrid.getState();
      repaint();
    } else if (ae.getSource() == eLvlAttrs) cmdELvlAttrs();
    else if (ae.getSource() == flipHoriz) flipHoiz();
    else if (ae.getSource() == flipVert) flipVert();
    else if (ae.getSource() == rotate) rotateByAngle();
    else if (ae.getSource() == retile) retile();
    else if (ae.getSource() == applyEzTile) applyEzTile();
    else if (ae.getSource() == randomFill) randomFill();
    else if (ae.getSource() == generateScript) doGenerateScript();
    else if (ae.getSource() == editUndo) undo();
    else if (ae.getSource() == editRedo) redo();
  ...
  public void eLVL_confirm() {
    if (elvlIcon == null)
      elvlIcon =
          new ImageIcon(
              Main.rootDirectory + File.separator + "include" + File.separator + "eLVLConfirm.png");

    Object[] options = {
      "Take me to the eLVL Editor", "No, I hate puppies", "No, and don't EVER ask me again"
    };
    int n =
        JOptionPane.showOptionDialog(
            null,
  ...
  public void help_tools() {
    File toolsHelpFile =
        new File(
            Main.rootDirectory + File.separator + "include" + File.separator + "toolsHelp.png");
    if (toolsHelpIcon == null) {
      toolsHelpIcon = new ImageIcon(toolsHelpFile.getAbsolutePath());
    }

    JOptionPane.showMessageDialog(
        null, "", "Tools Help", JOptionPane.INFORMATION_MESSAGE, toolsHelpIcon);
  }
  ...
  public void help_about() {
    JOptionPane.showMessageDialog(null, "Continuum Level / Lvz Tool " + VERSION + " by Team 12!");
  }

  public void copy() {
    Editor e = (Editor) contentManager.getComponent(contentManager.getSelectedIndex());
  ...
  public void setStyle(int num) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          null, "You do not have that style installed.\n" + ea.toString());
    }
  }
  ...
  private void saveProjectState(ProjectState ps) { // saves preferences for next time
    ...
    if (lastMaps[0] != null) {
      ...
      if (lastMaps[1] != null) {
        ...
        if (lastMaps[2] != null) {
          pref.addChild("last_map_2", "" + lastMaps[2].replace(' ', '?'));
        }
      }
    }

    XMLNode.saveDocument(
        document,
        Main.rootDirectory + File.separator + "include" + File.separator + "preferences.xml");
  }
    ...
    for (int x = 0; x < contentManager.getComponentCount(); ++x) {
      ...
      switch (s) {
        case JOptionPane.YES_OPTION:
          {
            if (e.m_fileName.equals(untitledMapName)) cmdSaveAs();
            else e.save();
          }
      }
    }
  ...
  /**
   * A button was selected on the toolbar, update everything that needs to know about it
   *
   * @param b the button that was pressed
   * @param button the integer representing that button
   */
  public void setButtons(JToggleButton b, int button) {
    ...
    for (int j = 0; j < ct; j++)
      ((Editor) contentManager.getComponentAt(j)).m_window.setTool(button);
  }
  ...
  private void showWindow(int window) {
    ...
    if (window == RADAR_WINDOW) windowRadar.setEnabled(false);
    else if (window == TILESET_WINDOW) windowTileset.setEnabled(false);
    else if (window == AUTOTILE_WINDOW) windowAutoTile.setEnabled(false);
    else if (window == LVZ_WINDOW) windowLvz.setEnabled(false);
    else if (window == REGION_WINDOW) windowAsssRegions.setEnabled(false);
  ...
  private void cmdSaveAll() {
    for (int x = 0; x < contentManager.getComponentCount(); ++x) {
      ...
      if (e != null) {
        if (e.m_lvlFile.m_file != null) e.save();
        else {
          ...
          if (filename.length() < 4 || !filename.substring(filename.length() - 4).equals(".lvl"))
            filename += ".lvl";
    ...
    if (e != null) {
      if (e.m_lvlFile.m_file != null) {
        ...
        if (confirmELVL
            ...
            && ((String) ((Vector) e.m_lvlFile.eLvlAttrs.get(0)).get(1))
                .equals("Unnamed")) { // if the user hasn't modified their eLVL tags for their new
          // map
          eLVL_confirm();
        }
  ...
  private void cmdSaveAs() {
    ...
    if (e != null) {
      ...
      if (path.length() < 4 || !path.substring(path.length() - 4).equals(".lvl")) path += ".lvl";
  ...
  private void cmdOpenFile() {
    ...
    if (filename.length() < 4 || !filename.substring(filename.length() - 4).equals(".lvl")) {
      JOptionPane.showMessageDialog(
          null, "The only types of files that you can open are .lvl files.");
      return;
    }
   ...
   *
   * @param f the file to add to the queue
   */
  ...
  private void loadFileFromPath(String path) {
    ...
    if (file.exists()) {
      ...
      JOptionPane.showMessageDialog(null, "That file does not exist: " + file.getAbsolutePath());
    }
  }
  ...
  private void cmdEditTileset() {
    ...
    for (int x = 0; x < 19; ++x) {
      for (int y = 0; y < 10; ++y) {
        ...
        g.drawImage(e.m_tileset, 0, 0, 16, 16, 16 * x, 16 * y, 16 * x + 16, 16 * y + 16, null);
      }
    }
  ...
  public int getSelectedTool() {
    if (buttonPen.isSelected()) return Tools.PEN;
    else if (buttonSelect.isSelected()) return Tools.SELECT;
    else if (buttonEyedrop.isSelected()) return Tools.EYEDROP;
    else if (buttonAutoPen.isSelected()) return Tools.AUTO_PEN;
    else if (buttonLvz.isSelected()) return Tools.LVZ;
    else if (buttonLvzSelection.isSelected()) return Tools.LVZ_SELECTION;
    else if (buttonZoom.isSelected()) return Tools.ZOOM;
  ...
  public static Image TileIdToImage(int tile) {
    if (tile == 191) return newTiles[0];
    else if ((tile >= 192 && tile <= 215)
        || (tile >= 221 && tile <= 240)
        || (tile >= 243 && tile <= 251)) {
      return newTiles[1];
    } else if (tile == 241) {
