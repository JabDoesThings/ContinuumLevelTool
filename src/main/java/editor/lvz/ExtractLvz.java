...
public class ExtractLvz {
  ...
  static final String INI_HEADER =
      "; Created by an ExtractLvz modification by BaK- version " + VERSION;
  ...
  public static boolean DoExtractLvz(String path) {
    ...
    try {
      ...
      if (!ok) {
        ...
        BufferedOutputStream iniOut =
            new BufferedOutputStream(new FileOutputStream(new File(iniLocation)));

        writeLine(iniOut, INI_HEADER);
        writeLine(iniOut, "; Created on " + new GregorianCalendar().getTime());
        writeLine(iniOut, "");
        ...
        for (int x = 0; x < count && rv == ERROR_NONE; ++x) { // for each compressed section
          bi.read(dword, 0, 4);
          ok = checkHeader(dword);

          if (!ok) rv = ERROR_COMPRESSEDHEADER;
          ...
          if (compressedSize != decompressedSize) { // is it compressed?
          ...
          if (aFile) {
            ...
            if (fileTime > 0) out.setLastModified(fileTime * 1000);
          } else
          // ini part
          {
            rv = doIni(result);
   ...
   *
   * @param data the uncompressed data for this section
  ...
  private static int doIni(byte[] data) {
    ...
    if (size < 12) rv = ERROR_TOO_SHORT_INI_SECTION;
    else {
      ...
      if (format == OBJTYPE_ERROR) rv = ERROR_OBJHEADER;
      else {
        ...
        for (int x = 0; x < objectCount && rv == ERROR_NONE; ++x) {
          ...
          if (mapObject || format == OBJTYPE_CLV1) {
            ...
            mo.id = (toInt(data[offset + 1]) << 7) | (toInt(data[offset]) >> 1);
            ...
            if (mapObject) mapobjs.add(mo);
            else screenobjs.add(mo);
          } else {
            ...
            so.id = (toInt(data[offset + 1]) << 7) | (toInt(data[offset]) >> 1);

            so.x_type = (data[offset + 2] & 0x0F);
            so.x_coord = ((data[offset + 2] & 0x0F0) >> 4) | (data[offset + 3] << 4);

            so.y_type = (data[offset + 4] & 0x0F);
            so.y_coord = ((data[offset + 4] & 0x0F0) >> 4) | (data[offset + 5] << 4);
            ...
            if (so.x_type > MAX_SCREEN_OBJ_OFFSET || so.y_type > MAX_SCREEN_OBJ_OFFSET) {
        ...
        if (mapobjs.size() > 0) {
          ...
          restOfIni.add(
              ";<X>, <Y>, <Image Id>, <Layer>, <Mode>, <Display Time (1/100th seconds)>, <Object Id>");
   ...
   */
  private static void writeLine(BufferedOutputStream out, String s) throws IOException {
    if (s.length() > 0) {
  ...
  /**
   * Get the name of this Lvz File: so like if f points to "c:\mylvz.lvz" we'd return "mylvz"
   *
   * @param f the file of the .lvz file
  ...
  private static String getLvzName(File f) {
    ...
    if (substringLength < 0) substringLength += 4;
   ...
   *
   * <p>Thx Dr. Brain for a bug fix in this function.
   *
   * @param f the lvz file
   */
  private static void makeDir(File f) {
    String name = getLvzName(f);
    String parent = f.getParent();

    if (parent != null) destDir = parent + File.separator;
   ...
   *
   * @param array the array to extact from
   * @param offeset the offset where we're extracting
   ...
   *
   * @param dword the header byte[], 4 bytes long
   * @return true iff the header is correct
   */
  private static boolean checkHeader(byte[] dword) {
    boolean rv = false;
    if (dword[0] == 'C' && dword[1] == 'O' && dword[2] == 'N' && dword[3] == 'T') rv = true;
   ...
   *
   * @param dword byte[] header is in, at least 4 bytes long
  ...
  private static int checkObjectSectionHeader(byte[] dword) {
    ...
    if (dword[0] == 'C' && dword[1] == 'L' && dword[2] == 'V') {
      if (dword[3] == '1') rv = OBJTYPE_CLV1;
      else if (dword[3] == '2') rv = OBJTYPE_CLV2;
    }
