...
public class LvzImageWindow extends JInternalFrame
  ...
  public LvzImageWindow(LevelWindow p, Point l) {
    ...
    l_displayTime = new JLabel("Display Time (seconds):", SwingConstants.RIGHT);
    ...
    s_displayTime = new JSpinner(new SpinnerNumberModel(0.0, 0.00, 320, 0.01));
    ...
    l_mo_displayTime = new JLabel("Display Time (seconds):", SwingConstants.RIGHT);
    ...
    s_mo_displayTime = new JSpinner(new SpinnerNumberModel(0.0, 0.0, 320.0, 0.01));
  ...
  public Image getSelectedImage() {
    if (imagePanel.selectedImage == -1) return null;

    Vector curImages = (Vector) imagePanel.imageFiles.get(lvzSelector.getSelectedIndex());
  ...
  public void deleteMapObject(MapObject o) {
    ...
    for (int layer = MapObject.BELOWALL; layer < MapObject.NUMLAYERS; ++layer) {
      for (int num = 0; num < imagePanel.imageFiles.size(); ++num) {
        ...
        for (int c = 0; c < mapObjs.size(); ++c) { // for each map object
  ...
  public void deleteSelection() {
    ...
    for (int num = 0; num < imagePanel.imageFiles.size(); ++num) {
      ...
      while (i.hasNext()) {
        ...
        if (x) parent.undoer.deleteMapObject(parent, mo);
      }
      updateMOgui();
      ...
      for (int num = 0; num < imagePanel.imageFiles.size(); ++num) {
        ...
        for (int c = 0; c < mapObjs.size(); ++c) { // for each map object
          ...
          if (r.contains(xPos, yPos)) {
            if (parent.m_parent.m_main.isSingleLVZSelectionMode()) selectedMO.clear();
  ...
  public void saveLvz(File lvlFile, ProgressFrame pf) {
    ...
    String lvzDirectory = name.substring(0, name.length() - 4) + " files" + File.separator;
    ...
    for (int x = 0; x < lvzSelector.getItemCount(); ++x) {
      ...
      for (int y = 0; y < images.size(); ++y) {
        ...
        // Image i = li.image;
        String filename =
            dir.getAbsolutePath() + File.separator + s + "_image" + y + li.image.getExtention();
      ...
      makeIniFile(
          dir.getAbsolutePath() + File.separator + s + ".ini", s + ".lvz", images, mapobjects, s);

      String thePath = dir.getAbsolutePath() + File.separator + s + ".ini";

      int ok = LvzFiling.buildLvz(thePath);

      if (ok == CreateLvz.ERROR_NONE) {
        String lvzLocation = dir.getAbsolutePath() + File.separator + s + ".lvz";
        LvzFiling.copyFile(lvzLocation, lvzDirectory + s + ".lvz");
      } else
        JOptionPane.showMessageDialog(
            null, "LVZ build failed/interrupted:\n" + CreateLvz.errors[ok]);
    }

    return;
  }

  public void makeIniFile(
      String iniFile, String output, Vector images, Vector mapobjects, String lvzName) {
    ...
    try {
      ...
      for (int x = 0; x < images.size(); ++x) {
        ...
        out.println("File=" + lvzName + "_image" + x + l.image.getExtention());
      }
      out.println("");
  ...
  public void deleteDirectory(File dir) {
    if (dir.isDirectory()) {
      ...
      for (int i = 0; i < dirFiles.length; i++) {
        if (dirFiles[i].isDirectory()) deleteDirectory(dirFiles[i]);
        else dirFiles[i].delete();
      }
  ...
  public void updateMOgui() {
    ...
    if (mapObjects.size() == 0) // no lvz
    {
      ...
      Vector selectedObjects = (Vector) mapObjects.get(c_mo_lvz.getSelectedIndex());
      ...
      // objects
      {
        ...
        s_mo_num.setModel(new SpinnerNumberModel(0, 0, selectedObjects.size() - 1, 1));

        Vector images = (Vector) imagePanel.imageFiles.get(c_mo_lvz.getSelectedIndex());

        if (images != null && images.size() > 0) {
          s_mo_imageId.setModel(new SpinnerNumberModel(0, 0, images.size() - 1, 1));
        }
  ...
  public void updateSelectedMapObject() {
    ...
    if (m.layer == MapObject.AFTERBACKGROUND) c_mo_layer.setSelectedItem("AfterBackground");
    else if (m.layer == MapObject.AFTERCHAT) c_mo_layer.setSelectedItem("AfterChat");
    else if (m.layer == MapObject.AFTERGAUGES) c_mo_layer.setSelectedItem("AfterGauges");
    else if (m.layer == MapObject.AFTERSHIPS) c_mo_layer.setSelectedItem("AfterShips");
    else if (m.layer == MapObject.AFTERTILES) c_mo_layer.setSelectedItem("AfterTiles");
    else if (m.layer == MapObject.AFTERWEAPONS) c_mo_layer.setSelectedItem("AfterWeapons");
    else if (m.layer == MapObject.BELOWALL) c_mo_layer.setSelectedItem("BelowAll");
    else if (m.layer == MapObject.TOPMOST) c_mo_layer.setSelectedItem("TopMost");

    if (m.mode == MapObject.DEATH) c_mo_mode.setSelectedItem("Death");
    else if (m.mode == MapObject.ENTERARENA) c_mo_mode.setSelectedItem("EnterArena");
    else if (m.mode == MapObject.ENTERZONE) c_mo_mode.setSelectedItem("EnterZone");
    else if (m.mode == MapObject.KILL) c_mo_mode.setSelectedItem("Kill");
    else if (m.mode == MapObject.SHOWALWAYS) c_mo_mode.setSelectedItem("ShowAlways");
    else if (m.mode == MapObject.SERVERCONTROLLED) c_mo_mode.setSelectedItem("ServerControlled");
  }
  ...
  public void stateChanged(ChangeEvent e) {
    if (e.getSource() == s_mo_num) {
      ...
      if (e.getSource() == s_mo_x) {
        ...
        if (selectedMO.size() == 0) selectedMO.put(m, new Rectangle());
        ...
        Vector im = (Vector) imagePanel.imageFiles.get(c_mo_lvz.getSelectedIndex());
        LvzImage l = (LvzImage) im.get(m.imageIndex);
        Rectangle r = new Rectangle(m.x, m.y, l.image.getWidth(null), l.image.getHeight(null));

        if (((Number) s_mo_y.getValue()).intValue() != m.y) parent.modified = true;
        ...
        m.displayTime = (int) (((Number) s_mo_displayTime.getValue()).doubleValue() * 100);
      ...
      if (!before.equals(m)) parent.undoer.setLvzProperties(parent, before, m);
    }
  }
  ...
  public void addMapObject(int x, int y) {
    ...
    o.displayTime = (int) (((Number) s_displayTime.getValue()).doubleValue() * 100);
    ...
    Vector selectedObjects = (Vector) mapObjects.get(lvzSelector.getSelectedIndex());
  ...
  public void paintUnderTilesLvz(
      Graphics g, int scale, int startx, int endx, int starty, int endy) {
    Rectangle view =
        new Rectangle(startx * 16, starty * 16, (endx - startx) * 16, (endy - starty) * 16);
  ...
  public void paintAboveTilesLvz(
      Graphics g, int scale, int startx, int endx, int starty, int endy) {
    Rectangle view =
        new Rectangle(startx * 16, starty * 16, (endx - startx) * 16, (endy - starty) * 16);
    ...
    if (parent.currentTool == Tools.SELECT || parent.currentTool == Tools.LVZ_SELECTION) {
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == b_mo_delete) {
      ...
      if (parent.m_parent.m_lvlFile.m_file == null) {
        ...
        if (rv == JOptionPane.YES_OPTION) parent.m_parent.m_main.cmdSave();
      }
      ...
      while (!ok) {
        ...
        name = (String) JOptionPane.showInputDialog(null, "LVZ Name (no .lvz):", name);
        ...
        for (int x = 0; x < lvzSelector.getItemCount(); ++x) {
          if (name.toLowerCase().equals(((String) lvzSelector.getItemAt(x)).toLowerCase())) {
            JOptionPane.showMessageDialog(
                null, "An LVZ file with this name is already being used by this map.");
        ...
        if (name.length() == 0) {
          ...
          JOptionPane.showMessageDialog(null, "The name cannot be \"\"");
        } else if (name.charAt(name.length() - 1) == '.') {
          JOptionPane.showMessageDialog(null, "The name cannot end in a period.");
          ok = false;
        }
      }
      ...
      if (i.getWidth(null) > 512 || i.getHeight(null) > 512) {
        ...
        String filename = path + File.separator + "temp" + File.separator + "temp.png";
      ...
      if (n == JOptionPane.NO_OPTION) // animation
      {
        ...
        do {
          String s = (String) JOptionPane.showInputDialog(null, "X Frames:", "1");
          ...
          try {
            ...
            if (xFrames > 0) ok = true;
            else ok = false;
          } catch (NumberFormatException er) {
            ok = false;
          }

          if (!ok) JOptionPane.showMessageDialog(null, "Please only enter positive whole numbers.");
        } while (!ok);

        do {
          String s = (String) JOptionPane.showInputDialog(null, "Y Frames:", "1");
        ...
        do {
          String s = (String) JOptionPane.showInputDialog(null, "Animation Time(seconds):", "1.0");
          ...
          try {
            ...
            if (animTime > 0) ok = true;
            else ok = false;
          } catch (NumberFormatException er) {
            ok = false;
          }

          if (!ok) JOptionPane.showMessageDialog(null, "Please only enter positive numbers.");
        } while (!ok);
      }
      ...
      for (int index = 0; index < selected.length; ++index) {
        ...
        if (n == JOptionPane.NO_OPTION) // animation
        {
          ...
          do {
            ...
            if (!ok)
              JOptionPane.showMessageDialog(null, "Please only enter positive whole numbers.");
          } while (!ok);
          ...
          do {
            String s =
                (String) JOptionPane.showInputDialog(null, "Animation Time(seconds):", "1.0");
  ...
  private String importLvz() {
    ...
    if (file.length() < 5 || !file.substring(file.length() - 4).equals(".lvz")) {
      JOptionPane.showMessageDialog(null, "Only files with a .lvz extentions are allowed.");
      return null;
    }

    if (parent.m_parent.m_lvlFile.m_file == null) {
      JOptionPane.showMessageDialog(
          null, "You must save the .lvl before you can import lvz files.");
      return null;
    }

    if (parent.m_parent.m_lvlFile.m_file.getName().length() < 4) {
      JOptionPane.showMessageDialog(null, "The .lvl file doesn't end in .lvl?");
      return null;
    }

    String lvlDirectory = parent.m_parent.m_lvlFile.m_file.getParent() + File.separator;
    String lvlName =
        parent
            .m_parent
            .m_lvlFile
            .m_file
            .getName()
            .substring(0, parent.m_parent.m_lvlFile.m_file.getName().length() - 4);

    String selectedFile = dir + file;
    String destination = lvlDirectory + lvlName + " files" + File.separator + file;
  ...
  private String importLvz(File file) // opens in place
    ...
    String noExtention = selectedFile.substring(0, selectedFile.length() - 4);

    String name = "";
    for (int x = noExtention.length() - 1; x >= 0; --x) {
      if (noExtention.charAt(x) == File.separatorChar) break;
    ...
    if (f.exists()) {
      ...
      if (images != null) {
        ...
        imagePanel.setPreferredSize(
            new Dimension(
                (images == null ? 0 : images.size()) * (imageSize + 5) + 5, imageSize + 40));
...
class ImagePanel extends JPanel implements MouseListener {
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (imageFiles != null) {
      if (imageFiles.size() == 0
          || ((Vector) imageFiles.get(parent.lvzSelector.getSelectedIndex())).size() == 0) {
      ...
      if (lastDisplayedImages != parent.lvzSelector.getSelectedIndex()) { // resize required
        lastDisplayedImages = parent.lvzSelector.getSelectedIndex();
        int index = parent.lvzSelector.getSelectedIndex();
        setPreferredSize(
            new Dimension(selectedImages.size() * (imageSize + 5) + 5, imageSize + 40));
        ...
        for (int x = 0; x < selectedImages.size(); ++x) {
          ...
          if (numFrames == 1) {
            ...
            g.drawImage(
                ...
                null); // source
            g.drawString(
                l.image.getWidth(null) + "x" + l.image.getHeight(null), xCoord, imageSize + 15);
          } else if (numFrames > 1 && (l.animationTime / numFrames) > 0) {
            g.drawString("Animation", xCoord, imageSize + 27);
            g.drawString(
                (l.image.getWidth(null) / l.xFrames) + "x" + (l.image.getHeight(null) / l.yFrames),
                xCoord,
                imageSize + 15);

            long centiseconds = (Calendar.getInstance().getTime().getTime()) / 10;
            long frame = (centiseconds / (l.animationTime / numFrames)) % numFrames;
            ...
            g.drawString((frame + 1) + "/" + numFrames, xCoord, imageSize + 39);
          }

          if (x == selectedImage) {
            Rectangle r = new Rectangle(xCoord - 2, yCoord - 2, imageSize + 4, imageSize + 4);
  ...
  public void addImage(LvzImage i) {
    ...
    setPreferredSize(new Dimension(selectedImages.size() * (imageSize + 5) + 5, imageSize + 40));
    parent.scrollPane.setViewportView(this);
  ...
  public void removeSelectedImage() {
    ...
    Vector mos = (Vector) parent.mapObjects.get(parent.lvzSelector.getSelectedIndex());
