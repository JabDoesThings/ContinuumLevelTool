...
public class LvzFiling {
   ...
   *
   * @param path the path to extract the lvz from
   ...
   */
  public static int buildLvz(String iniFile) { // this builds it to the local buildlevel directory
  ...
  static boolean copyFile(String from, String to) {
    ...
    if (from != to) {
      try {
        ...
        if (!f.exists()) {
          if (!f.mkdirs()) throw new IOException("mkdirs failed on " + f.getAbsolutePath());
        }
  ...
  static Vector loadImages(String iniFile) // returns a vector of LvzImages
    ...
    try {
      ...
      while (br.ready()) {
        ...
        if (line.equals("[mapobjects]")) break;
  ...
  static Vector loadMapObjects(String iniFile) // returns a vector of
    ...
    try {
      ...
      while (true) {
        ...
        if (line.equals("[screenobjects]")) break;
        ...
        if (line.length() > 1 && !line.substring(0, 1).equals(";")) {
          ...
          if (s.length > 6) {
            ...
            m.imageIndex = Integer.parseInt(s[2].trim().substring((5)));
            ...
            System.err.println("error, split resulted in < 7 substrings line =" + line);
            System.exit(0);
          }
        }
      }
