...
// right?
{
  ...
  private int[] getAllColors() {
    ...
    PixelGrabber pg = new PixelGrabber(drawHere, 0, 0, 512, 512, storeHere, 0, 512);
    try {
      pg.grabPixels();
  ...
  private void fillAtPoint(Dimension d, Point start, Graphics g, int sourcePixel, int[] pixels) {
    ...
    Rectangle r = new Rectangle(512 / 2 - d.width / 2, 512 / 2 - d.height / 2, d.width, d.height);
  ...
  private Image draw() {
    ...
    for (int x = 0; x < curFrame; ++x) {
      ...
      if (one.getClass() == Color.class) { // color
        g.setColor((Color) one);
      } else if (one.getClass() == Line2D.Float.class) { // line/pencil
        ++x;
        Stroke two = (Stroke) l.get(x);
        g.setStroke(two);

        g.draw((Line2D.Float) one);
      } else if (one.getClass() == Rectangle.class) { // rectangle
        ++x;
        Stroke two = (Stroke) l.get(x);
        g.setStroke(two);
        ++x;
        ...
        if (three.booleanValue() == false) g.draw((Rectangle) one);
        else g.fill((Rectangle) one);
      } else if (one.getClass() == Ellipse2D.Float.class) { // oval
        ++x;
        Stroke two = (Stroke) l.get(x);
        g.setStroke(two);
        ++x;
        ...
        if (three.booleanValue() == false) g.draw((Ellipse2D.Float) one);
        else g.fill((Ellipse2D.Float) one);
      } else if (one.getClass() == String.class) { // text
        ++x;
      ...
      } else if (one.getClass() == Point.class) { // Fill
        ++x;
        ...
        if (allPixels != null)
          fillAtPoint(d, imageP, g, allPixels[imageP.x + imageP.y * 512], allPixels);
      } else
      ...
      { // Selection\
        ...
        g.drawImage(selectedImage, selectedPoint.x, selectedPoint.y, null);
        // }
