...
public class BitMap extends JPanel {
  ...
  public boolean hasELVL = false; // since ELVL headers are so interlocked
  // with the bitmap, this is
  ...
  public void readBitMap(boolean trans) {
    ...
    if (m_bitCount <= 8) {
      ...
      for (int i = 0; i < m_colorsUsed; i++) {
        ...
        // Make black transparent. SS specific need, will adjust to be
        // dynamic
        if (m_colorTable[i] == 0xff000000 && trans) m_colorTable[i] = m_colorTable[i] & 0x00000000;
      }
    }

    if (m_compressionType == BI_RGB && m_bitCount <= 8) readInRGB();
    else if (m_compressionType == BI_RLE8 && m_bitCount == 8) readInRLE8();
  }

  public void readInRGB() {

    int shift[] = new int[8 / m_bitCount];
    for (int i = 0; i < 8 / m_bitCount; i++) shift[i] = 8 - ((i + 1) * m_bitCount);
  ...
  /**
   * Reads in a square tile of any size from the topleft Good for getting the first image in any of
   * the /graphics/*.bm2
   */
