...
public class LevelFile extends JPanel {
   ...
   * @param b The tileset bitmap (note read in default bitmap if lvl file does not contain bitmap
   *     portion)
   * @param had If the lvl file contained the tileset bitmap
   */
  public LevelFile(File file, BitMap b, boolean hasBMP, boolean hasELVL) {
    m_file = file;
    m_bitmap = b;
    m_containsBM = hasBMP;
    hasELVLData = hasELVL;
    ...
    try {
      m_stream = new BufferedInputStream(new FileInputStream(m_file));
    } catch (FileNotFoundException e) {
      JOptionPane.showMessageDialog(null, "Error opening input stream: " + e);
    }
  }
  ...
  public void addDefaultELvLTags() {
    ...
    row = new Vector();
    row.add("PROGRAM");
    row.add("Continuum Level Tool v" + Main.VERSION);
    eLvlAttrs.add(row);
  }
  ...
  private String readELvlData() {
    ...
    if (!available(12)) {
      ...
      if (!headerArray.readString(0, 4).equals("elvl")) {
        ...
        int size = headerArray.readLittleEndianInt(4); // total size of
        // the metadata
        // section
        ...
        while (current < size && error == null) {
          if (available(8)) {
            ...
            type[0] = curData.readByte(0);
            type[1] = curData.readByte(1);
            type[2] = curData.readByte(2);
            type[3] = curData.readByte(3);

            int chunkLength = curData.readLittleEndianInt(4);

            // System.out.println("Read chunk of type " +
            //		curData.readString(0, 4) + " len = " + chunkLength);

            if (!available(chunkLength)) {
              error =
                  "EOF while reading in an eLVL chunk of type(decimel) "
                      + type[0]
                      + " "
                      + type[1]
                      + " "
                      ...
                      + chunkLength
                      + ")"
                      + "\nAscii type (may be corrupted) = \""
                      + curData.readString(0, 4)
                      + "\"";
            ...
            // unknown chunk
            {
              ...
              // System.out.println("read unknown chunk of length "
              //		+ chunkLength);

              // encode header
              unknownELVLData.add(new Byte(type[0]));
              unknownELVLData.add(new Byte(type[1]));
              unknownELVLData.add(new Byte(type[2]));
              unknownELVLData.add(new Byte(type[3]));
              byte[] dword = BitmapSaving.toDWORD(chunkLength);
              for (int c = 0; c < 4; ++c) unknownELVLData.add(new Byte(dword[c]));
  ...
  /**
   * Actually do the read on the .lvl file, ignoring the already loaded bitmap part
   *
   * @return null or the error message
   */
  public String readLevel() {
    if (hasELVLData) readIn(m_bitmap.ELvlOffset);
    else if (m_containsBM) readIn(m_bitmap.getFileSize());
    ...
    try {
      ...
      JOptionPane.showMessageDialog(null, "Error closing input stream: " + e);
    }
   ...
   */
  public void saveLevelAs(File where, Image tileset, short[][] map, Vector regions) {
    m_file = where;
    saveLevel(tileset, map, regions);
  }

  /**
   * make the eLVL data and store it as a Vector of Bytes in eLVLData, not including the header
   *
  ...
  private void makeELvlDataForSaving(Vector regions) {
    ...
    for (int x = 0; x < size; ++x) {
      ...
      for (int c = 0; c < 4; ++c) eLVLData.add(new Byte(chunkSizeBytes[c]));
    ...
    for (int x = 0; x < regions.size(); ++x) {
      ...
      for (int c = 0; c < 4; ++c) eLVLData.add(new Byte(dword[c]));
  ...
  /**
   * Save the ELVL data to the current position in the stream. It's stored in a vector of Bytes in
   * eLVLData
   *
  ...
  public boolean available(int n) {
    try {
      if (m_stream.available() >= n) return true;
      else return false;
    } catch (IOException e) {
      System.out.println(e);
      return false;
