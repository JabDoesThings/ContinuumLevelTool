...
public class ASSSRegionWindow extends JInternalFrame
   ...
   *
   * @param whichOne which region was selected(row)? or -1
   */
  ...
  public void addRectangleToCurrentRegion(Rectangle rect) {
    ...
    if (index == -1) {
      if (createRegion()) index = tableModel.data.size() - 1;
      else return;
    }
    ...
    if (rects.getItemCount() > 0) s = (String) rects.getItemAt(rects.getItemCount() - 1);
    else s = "Rectangle0";
   ...
   *
   * @param newRegions the new regions we're adding
   */
  ...
  public void loadRegions(XMLNode regions) {
    ...
    for (int x = 0; x < regions.children.size(); ++x) {
      ...
      if (baseNode != null && baseNode.getBooleanValue() == true) base = true;

      int r = 128, g = 128, b = 128;
      XMLNode colorNode = aRegion.getChild("Color");
      if (colorNode != null) {
        XMLNode redNode = colorNode.getChild("Red");
        if (redNode != null) r = redNode.getIntValue();
        XMLNode greenNode = colorNode.getChild("Green");
        if (greenNode != null) g = greenNode.getIntValue();
        XMLNode blueNode = colorNode.getChild("Blue");
        if (blueNode != null) b = blueNode.getIntValue();
      }
      ...
      for (int c = 0; c < aRegion.children.size(); ++c) {
        ...
        if (nodeName.length() > 9) {
          if (nodeName.substring(0, 9).equals("Rectangle")) {
            ...
            if (xNode != null) X = xNode.getIntValue();

            XMLNode yNode = aNode.getChild("Y");
            if (yNode != null) Y = yNode.getIntValue();

            XMLNode wNode = aNode.getChild("Width");
            if (wNode != null) W = wNode.getIntValue();

            XMLNode hNode = aNode.getChild("Height");
            if (hNode != null) H = hNode.getIntValue();
  ...
  public boolean createRegion() {
    ...
    do {
      if (repeat == true) {
        JOptionPane.showMessageDialog(null, "That name is already used by another region.");
        repeat = false;
      } else if (currentString != null && currentString.equals("")) {
        JOptionPane.showMessageDialog(null, "You must supply a region name.");
      }

      currentString = JOptionPane.showInputDialog(null, "Enter New Region Name:", currentString);
    ...
    } while (repeat == true || (currentString != null && currentString.equals("")));

    if (currentString == null) return false;

    Region r = new Region(currentString, ASSSFiling.getColorForRow(tableModel.data.size()));

    tableModel.data.add(r);
    table.revalidate();
    table
        .getSelectionModel()
        .setSelectionInterval(tableModel.data.size() - 1, tableModel.data.size() - 1);
    repaint();
  ...
  private void populateComboBox(int row) {
    if (tableModel.data.size() > row) {
      ...
      if (r.rects.size() > 0) enableSingleRectGUI();
      else disableSingleRectGUI();
    }
  }
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == importRgn) importRegionFile();
    else if (e.getSource() == add) createRegion();
  ...
  private void updateSpinners(int rectNum) {
    ...
    if (row == -1) System.out.println("WTF? row == -1 while a rectangle is being selected?");
    else {
      Region r = (Region) tableModel.data.get(row);
   ...
   *
   * @param g the grapgics to paint with
   * @param scale the pixels per tile
   * @param startx the startx of our drawing arena
   * @param endx the endx of our drawing area
   * @param starty the starty of our drawing area
   * @param endy the endy of our drawing area
   */
  public void paintRegions(Graphics g, int scale, int startx, int endx, int starty, int endy) {
    Rectangle view =
    ...
    for (int row = 0; row < table.getRowCount(); ++row) {
      ...
      for (int x = 0; x < r.rects.size(); ++x) {
        ...
        Rectangle port = new Rectangle(rect.x * 16, rect.y * 16, rect.width * 16, rect.height * 16);
   ...
   * @param endy the endy of our drawing area
   */
  public void paintSelectedRegion(
  ...
  public class StringEditor extends AbstractCellEditor implements TableCellEditor, MouseListener {
    ...
    public StringEditor() {
      ...
      label.setBorder(BorderFactory.createMatteBorder(1, 1, 1, 1, table.getSelectionBackground()));
    ...
    public void mouseClicked(MouseEvent e) {
      if (e.getClickCount() == 2) {
        ...
        do {
          if (repeat == true) {
            JOptionPane.showMessageDialog(label, "That name is already used by another region.");
            repeat = false;
          }

          String rv = JOptionPane.showInputDialog(label, "Enter Region Name:", currentString);
          ...
          if (rv != null) // didn't press cancel
          {
            ...
            if (repeat == false) currentString = rv;
          }
        } while (repeat == true);
  ...
  public class ColorEditor extends AbstractCellEditor implements TableCellEditor, MouseListener {
    ...
    public ColorEditor() {
      label = new JLabel();
      label.setOpaque(true);
      label.setBorder(BorderFactory.createMatteBorder(2, 5, 2, 5, table.getSelectionBackground()));
      label.addMouseListener(this);
    }
    ...
    public void mouseClicked(MouseEvent e) {
      if (e.getClickCount() == 2) {
        ...
        Color c = JColorChooser.showDialog(label, "Choose Color for Region", currentColor);
  ...
  class MyTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int col) {
      if (col == 0) return ((Region) data.get(row)).name;
    ...
    public void setValueAt(Object value, int row, int col) {
      ...
      if (col == 0) r.name = (String) value;
      else if (col == 1) r.color = (Color) value;
